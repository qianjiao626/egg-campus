...
- 学校种子使用附件全量 `2361` 条，包含本科、专科和高职；演示阶段可按用户要求导入附件中的 16 条演示吐槽和评分。
...
视觉重做沿用附件原版的奶油粉橙方向：顶部“吐槽最狠/吐槽最少”极值卡片，16 项指标 Tab，左侧排名列表与状态徽章，右侧实时滚动吐槽墙；搜索联想、新增学校、学校详情、单项排名和评分提交弹窗均保留附件交互。数据全部由现有 API 提供，不恢复前端 mock 数据。
