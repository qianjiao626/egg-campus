...
- Produces: 开发部署完成后连续三轮测试报告；每轮包含测试时间、用例项、预期结果、实际结果、是否通过、问题简述。

- [ ] **Step 1: 创建独立 Codex 测试任务**

使用 Codex `create_thread` 创建用户可见的独立测试任务。该任务不做定时轮询，先等待主任务完成开发和部署；收到“开始三轮检测”后连续执行三轮完整测试。提示词要求只写测试报告和回传问题，不修改源码、不部署、不调用实现子智能体。

- [ ] **Step 2: 写入三轮测试模板**

每轮必须覆盖：刷新后个人页身份、完整选填注册、任务广场、角色档案、零角标、WebSocket 降级、盲盒、吐槽、排行榜、邀请码。未测试项目写“阻塞”及原因，不能伪造通过；三轮之间必须重新加载页面并重新读取服务端状态。
...
- [ ] **Step 6: 开发部署完成后触发独立测试任务**

向独立测试任务发送“开始三轮检测”。读取三轮报告；任一轮失败都在当前任务修复并重新部署，然后让测试任务从第一轮重新开始。不得把未验证项标成通过。
