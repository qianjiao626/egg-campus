...
- [ ] 写测试：普通用户公开查询只得到 `approved`；审核者队列得到 `pending_review`、`needs_revision`、`rejected`；本人查询只按 `userId`。
- [ ] 运行 `npx vitest run tests/task-visibility.test.ts`，确认因模块不存在失败。
- [ ] 实现纯函数并补 Prisma 字段和非商城迁移。
- [ ] 运行目标测试与 `npx prisma validate`。
...
- [ ] 写 Fastify 契约测试，覆盖公开、审核、本人三种可见性及创建/编辑技能字段。
- [ ] 写接取与放弃测试，断言数据库聚合人数和当前用户接取标记，不接受前端本地计数。
- [ ] 运行目标测试，确认新路由/字段缺失导致失败。
- [ ] 修改 Zod schema、任务序列化、路由查询和事务。
- [ ] 运行任务契约测试及现有 `business-contract.test.ts`。
...
- [ ] 写测试覆盖无效邀请码、自邀、重复绑定、首次审核奖励 `20`、重复审核不奖励。
- [ ] 运行测试确认失败。
- [ ] 增加 `Invitation` 模型、唯一约束、注册事务绑定和审核事务奖励。
- [ ] 增加邀请码汇总/历史接口，所有 BigInt 序列化为字符串。
- [ ] 运行邀请测试和认证会话测试。
...
- [ ] 写测试覆盖 Zod 参数错误、重复昵称、邀请码错误和模拟数据库异常不泄漏堆栈。
- [ ] 补前端契约测试，断言密码提示“密码不低于8位”且非法参数不调用 fetch。
- [ ] 运行目标测试确认失败。
- [ ] 完成 Fastify 错误映射和前端中文提示；保留 HttpOnly refresh cookie 方案。
- [ ] 运行认证相关全部测试。
...
- [ ] 写文本契约测试，精确禁止生产 Demo 数组、假任务卡、假排行榜用户、假邀请码历史和本地计数回写。
- [ ] 运行测试并确认命中现有静态数据而失败。
- [ ] 删除静态业务记录及其合并逻辑，保留固定空状态和渲染函数。
- [ ] 接入真实任务、榜单、树洞、个人资产与邀请历史接口。
- [ ] 运行前端契约测试和盲盒内容契约测试。
...
- [ ] 写 DOM 文本契约测试，断言中部长条不存在、顶部身份节点存在、MPI 组件仍存在。
- [ ] 运行测试确认失败。
- [ ] 原位移动动态绑定节点并调整响应式 CSS，不创建嵌套卡片。
- [ ] 运行布局契约和现有资料更新测试。
...
- [ ] 安装 `@fastify/websocket` 与 `ws` 并写事件路由测试。
- [ ] 测试未认证连接拒绝、公共事件只给登录用户、管理员事件检查实时 RBAC、私有事件只给目标用户。
- [ ] 运行测试确认实现缺失而失败。
- [ ] 实现连接注册、断开清理、事件广播和关闭钩子。
- [ ] 在任务、盲盒、树洞和资产变更成功后发布事件，任何事件失败不得回滚已提交 REST 业务。
- [ ] 运行实时测试与后端全套测试。
...
- [ ] 写契约测试覆盖全局开关、有限指数退避、一次性错误日志、REST 全量降级和取消订阅。
- [ ] 运行测试确认文件不存在而失败。
- [ ] 实现单连接客户端，并在页面进入/离开时注册和销毁监听。
- [ ] 将事件映射到现有 REST 加载函数，禁止 WebSocket 直接修改业务状态。
- [ ] 运行前端契约测试。
...
- [ ] 写纯规则与事务客户端测试，断言白名单缺失、额外账号、身份错误时拒绝执行。
- [ ] 运行测试确认失败。
- [ ] 实现显式确认、事务删除顺序、账户重置和唯一初次上线流水。
- [ ] 在测试数据库运行两次：第一次成功，第二次保持相同结果，证明幂等。
- [ ] 查询验证只剩三个账号、余额均为 `100`、每人一条流水。
...
- [ ] 本机运行 `npm test -- --run`、`npm run build`、`npx prisma validate` 和 `git diff --check`。
- [ ] 使用磁盘私钥创建测试库并部署独立测试进程；不打印连接串或秘密。
- [ ] 迁移测试库并通知独立测试会话开始首轮完整测试。
- [ ] 修复测试报告中的全部问题，重复测试直到全通过。
- [ ] 本机生成前端产物并增量上传生产服务器，保留服务器 `node_modules`。
- [ ] 校验 Nginx WebSocket Upgrade 配置后重载，重启后端并检查 PM2 日志、Nginx、健康接口、REST 和 WebSocket。
- [ ] 执行已授权生产清理命令并做最终数据库核验。
- [ ] 更新 Obsidian 变更日志，记录问题原因、修复和部署验证结果。
