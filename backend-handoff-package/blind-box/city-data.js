// 中国地级市及同级行政区（省/自治区/直辖市分组）。数据内置，嵌入后无需额外请求。
const cityGroups = {
...
function initializeRegions(data) {
  window.regionData = data;
  appendOptions(provinceSelect, Object.keys(data));
}

provinceSelect.addEventListener('change', () => {
  resetSelect(citySelect, '请先选择省份');
  resetSelect(districtSelect, '请先选择城市');
  const cities = Object.keys(window.regionData?.[provinceSelect.value] || {});
...
citySelect.addEventListener('change', () => {
  resetSelect(districtSelect, '请先选择城市');
  const districts = window.regionData?.[provinceSelect.value]?.[citySelect.value] || [];
  if (!districts.length) return;
  appendOptions(districtSelect, districts);
