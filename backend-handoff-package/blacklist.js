(function () {
  ...
  function wallCard(row) {
    ...
    var certified = row.isCertified ? '<span class="certified-badge" title="蛋总认定">✓</span>' : '';
    return '<article class="bl-wall-card"><div class="bl-wall-card-head"><span class="bl-wall-card-school">🏫 ' + esc(row.displayName || row.schoolName || '') + '</span><span class="bl-wall-card-score" style="color:' + scoreColor(score) + ';background:' + scoreBg(score) + '">' + score.toFixed(1) + ' 分</span><time class="bl-wall-card-time">' + esc(formatTime(row.createdAt || row.time)) + '</time></div><div class="bl-wall-card-text">' + esc(row.content || row.text || '仅留下评分') + '</div><div class="bl-wall-card-user">— ' + esc(row.nickname || row.userName || '匿名用户') + certified + '</div></article>';
  }
  ...
  async function openSchool(id) {
    ...
    try {
      ...
      var html = '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:22px"><div><div style="font-size:22px;font-weight:800;color:#FF6B9D">🏫 ' + esc(result.displayName || school.displayName || school.schoolName) + '</div><div style="font-size:13px;color:rgba(93,64,55,.5);margin-top:6px">' + (Number(result.count) || 0) + ' 条吐槽</div></div><div style="text-align:right"><div style="font-size:30px;font-weight:900;color:' + scoreColor(avg) + '">' + avg.toFixed(1) + '</div><div style="font-size:12px;color:rgba(93,64,55,.45)">平均吐槽分</div></div></div><div style="font-size:14px;font-weight:800;color:#5D4037;margin-bottom:14px">📋 各项指标</div>';
      ...
      html += '<div style="font-size:14px;font-weight:800;color:#5D4037;margin:22px 0 12px">💬 本校吐槽留言</div>'; var comments = result.comments || []; html += comments.length ? comments.slice(0, 10).map(function (row) { var score = Number(row.averageScore || 0); var certified = row.isCertified ? '<span class="certified-badge" title="蛋总认定">✓</span>' : ''; return '<article class="bl-detail-comment"><div class="bl-detail-comment-head"><span class="bl-detail-comment-user">' + esc(row.nickname || '匿名用户') + certified + '</span><span class="bl-detail-comment-score" style="color:' + scoreColor(score) + ';background:' + scoreBg(score) + '">吐槽分：' + score.toFixed(1) + '</span></div><div class="bl-detail-comment-text">' + esc(row.content || '仅留下评分') + '</div><div class="bl-detail-comment-time">' + esc(formatTime(row.createdAt)) + '</div></article>'; }).join('') : '<div class="bl-detail-comment" style="text-align:center;color:rgba(93,64,55,.45)">暂无留言</div>'; body.innerHTML = html;
